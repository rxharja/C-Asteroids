...
Body create_body(const Polygon shape, const Vector2 position, const Vector2 velocity,
                 const double angle, const double angular_velocity,
                 const double drag, const double mass) {
    return (Body){
        .shape = shape,
        .position = position,
        .velocity = velocity,
...
// semi-implicit Euler: apply acceleration, damp, move, rotate
...
int is_colliding_sat(const Body *a, const Body *b) {
    if (!test_axes(&a->shape, &b->shape)) return 0;
    if (!test_axes(&b->shape, &a->shape)) return 0;
    return 1;
}
